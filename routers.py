...
async def upload_education_file(file: UploadFile = File(...)):
    ...
    if len(file_bytes) == 0:
        return JSONResponse(
            status_code=422,
            content=BusynessErrorResponse(message="Файл пустой").model_dump(
                mode="json"
            ),
        )
    ...
    try:
        ...
        return JSONResponse(
            status_code=422,
            content=BusynessErrorResponse(
                message=f"Файл не является валидным PDF документом, {e}"
            ).model_dump(mode="json"),
        )
    ...
    return {"message": "Файл успешно загружен", "educationFilename": file.filename}
...
@router.post(
    ...
    responses={
        ...
        422: {
            "model": BusynessErrorResponse,
            "description": "Ответ документ об образовании не найден",
        },
    },
)
async def reserve_selection(selection_context: SelectionContext):
    ...
    if not os.path.exists(f"{STORAGE_DIR}/{selection_context.educationFilename}"):
        return JSONResponse(
            status_code=422,
            content=BusynessErrorResponse(message="Файл диплома не найден").model_dump(
                mode="json"
            ),
        )
    ...
    speciality_result = await check_resume_specialties(
    ...
    valid_education_document = doc_answer.type in (
        EducationDocType.DIPLOMA,
        EducationDocType.HIGHER_EDU_СERT,
    )
    education_match = speciality_result and (
        doc_answer.code == speciality_result[0].code
        or doc_answer.name == speciality_result[0].name
    )
    education_in_list = doc_answer.name in required_specialties or (
        speciality_result and speciality_result[0].name in required_specialties
    )
    no_violated_rules = len(moderation_result.violatedRules) == 0
    overall_success = (
        valid_education_document
        and education_match
        and education_in_list
        and no_violated_rules
    )
