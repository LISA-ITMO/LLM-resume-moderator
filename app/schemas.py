...
class Reletive(BaseModel):
    ...
    job: str = Field(
        ..., description="Место работы (наименование и адрес организации), должность", example="АО «Газпром», инженер"
    )
    address: str = Field(
        ...,
...
class Education(BaseModel):
    ...
    institutionName: str = Field(
        ..., description="Учебное заведение", example="Санкт-Петербургский государственный университет"
    )


class HigherEducation(Education):
    specialty: str = Field(
        ..., description="Специальность (направление подготовки), код", example="01.03.02 Прикладная математика и информатика"
    )
...
class Postgraduate(Education):
    specialty: str = Field(
        ..., description="Наименование направления или специальности по документу", example="Информационные технологии"
    )
...
class ResumeToGovernment(BaseModel):
    ...
    fullnameChange: Optional[str] = Field(
        None,
        description="Изменение Ф.И.О. (указать старое Ф.И.О., время, место и причину изменения)",
    ...
    registrationAddress: str = Field(
        ..., description="Адрес регистрации", example="г. Санкт-Петербург, ул. Невский пр., д. 12, кв. 34"
    )
    actualResidenceAddress: str = Field(
        ..., description="Адрес фактического проживания", example="г. Санкт-Петербург, ул. Ленина, д. 5"
    )
    contactInformation: str = Field(
        ..., description="Контактная информация (телефоны, e-mail)", example="+7 (921) 123-45-67, vladimir@mail.ru"
    )
    ...
    publications: List[str] = Field(
        ..., description="Научные труды (публикации) или изобретения", example=["Применение ML в госуправлении"]
    )
    ...
    militaryLiable: bool = Field(
        ..., description="Отношение к воинской обязанности (военнообязанный/не военнообязанный)", example=True
    )
    ...
    professionalInterests: str = Field(
        ..., description="Сфера профессиональных интересов", example="Анализ данных, финансы, образование"
    )
    ...
    source: str = Field(
        ..., description="Откуда узнали о резерве", example="Информация в вузе (центр карьеры, ярмарка вакансий)"
    )


class SelectionContext(BaseModel):
    rules: List[Rule] = Field(
        default=DEFAULT_RULES, description="Список правил на которые нужно проверить резюме", example=DEFAULT_RULES
    )
    resume: ResumeToGovernment = Field(..., description="Резюме которое нужно проверить")
    moderation_model: Optional[ModerationModel] = Field(
        ..., description="LLM модель которая будет проверять резюме на соответствие правилам", example=ModerationModel.T_it_1_0
    )
