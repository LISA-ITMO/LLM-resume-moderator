...
DEFAULT_RULES = [
    Rule(**rule) for rule in json.load(open("resume_rules.json", encoding="utf-8"))
]
...
class FinalResponse(ResponseWithReasoning):
    time_ms: int
    specialties_check: List = Field(
        default_factory=list, description="Результаты проверки специальностей"
    )
...
class ResumeToGovernment(BaseModel):
    ...
    snils: str = Field(
        ...,
        description="Страховой номер индивидуального лицевого счёта (СНИЛС)",
    ...
    militaryLiable: bool = Field(
        ...,
        description="Отношение к воинской обязанности (военнообязанный/не военнообязанный)",
        example=True,
    )
...
class SelectionContext(BaseModel):
    ...
    moderation_model: Optional[ModerationModel] = Field(
        ...
        example=ModerationModel.T_it_1_0,
    )
